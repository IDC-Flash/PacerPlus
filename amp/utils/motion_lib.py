...
class MotionLib():
    def __init__(self, motion_file, dof_body_ids, dof_offsets, key_body_ids,
        ...
        # self.torch_humanoid = Humanoid_Batch()  ### ZL SMPL Hack

        self._load_motions(motion_file)
        motions = self._motions
        self.gts = torch.cat([m.global_translation for m in motions],
                             dim=0).float()
        self.grs = torch.cat([m.global_rotation for m in motions],
                             dim=0).float()
        self.lrs = torch.cat([m.local_rotation for m in motions],
                             dim=0).float()
        self.grvs = torch.cat([m.global_root_velocity for m in motions],
                              dim=0).float()
        self.gravs = torch.cat(
            [m.global_root_angular_velocity for m in motions], dim=0).float()
    ...
    def sample_motions(self, n):
        motion_ids = torch.multinomial(self._motion_weights,
                                       num_samples=n,
                                       replacement=True)
    ...
    def get_motion_state_smpl(self, motion_ids, motion_times):
        n = len(motion_ids)
        ...
        vals = [
            root_pos0, root_pos1, local_rot0, local_rot1, root_vel,
            root_ang_vel, rg_pos0, rg_pos1
        ]
        for v in vals:
    ...
    def get_motion_state(self, motion_ids, motion_times):
        ...
        vals = [
            root_pos0, root_pos1, local_rot0, local_rot1, root_vel,
            root_ang_vel, key_pos0, key_pos1
        ]
        for v in vals:
            assert v.dtype != torch.float64
    ...
    def _load_motions(self, motion_file):
        ...
        for f in tqdm(range(num_motion_files)):
            curr_file = motion_files[f]

            if (isinstance(curr_file, str)):
                print("Loading {:d}/{:d} motion files: {:s}".format(
            ...
            if "beta" in motion_file_data:
                beta, gender, pose_aa = motion_file_data[
                    'beta'], motion_file_data['gender'], motion_file_data[
                        'pose_aa']
                ...
                seq_len, _, _ = curr_file['rotation']['arr'].shape
                self._motion_bodies.append(np.zeros(17))
                self._motion_quat.append(np.zeros([seq_len, 96]))
                self._motion_aa.append(np.zeros([seq_len, 72]))
            ...
            # jp hack
            #self._hack_test_vel_consistency(curr_motion)
        ...
        self._motion_fps = torch.tensor(self._motion_fps,
                                        device=self._device,
                                        dtype=torch.float32)
        self._motion_bodies = torch.tensor(self._motion_bodies,
                                           device=self._device,
                                           dtype=torch.float32)
        self._motion_aa = torch.tensor(np.concatenate(self._motion_aa),
                                       device=self._device,
                                       dtype=torch.float32)
        self._motion_quat = torch.tensor(np.concatenate(self._motion_quat),
                                         device=self._device,
                                         dtype=torch.float32)
        ...
        # self.torch_humanoid.update_model(self._motion_bodies[:, 1:],
        #                                  self._motion_bodies[:, 0])
    ...
    def _fetch_motion_files(self, motion_file):
        ...
        if (ext == ".yaml"):
            ...
            # motion_lens = [v['rotation']['arr'].shape[0] for v in motion_files]
            # motion_weights = np.array(motion_lens)/np.sum(motion_lens)
    ...
    def _compute_motion_dof_vels(self, motion):
        ...
        for f in range(num_frames - 1):
            ...
            frame_dof_vel = self._local_rotation_to_dof_vel(
                local_rot0, local_rot1, dt)
    ...
    def _local_rotation_to_dof(self, local_rot):
        ...
        n = local_rot.shape[0]
        dof_pos = torch.zeros((n, self._num_dof),
                              dtype=torch.float,
                              device=self._device)
        ...
        for j in range(len(body_ids)):
            ...
            if (joint_size == 3):
                ...
                dof_pos[:, joint_offset:(joint_offset +
                                         joint_size)] = joint_exp_map
            elif (joint_size == 1):
                joint_q = local_rot[:, body_id]
                joint_theta, joint_axis = torch_utils.quat_to_angle_axis(
                    joint_q)
                joint_theta = joint_theta * joint_axis[
                    ..., 1]  # assume joint is always along y axis
