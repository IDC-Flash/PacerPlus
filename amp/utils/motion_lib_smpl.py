...
def fix_trans_height(pose_aa, trans, curr_gender_betas, smpl_parsers):
    with torch.no_grad():
        ...
        #diff_fix  = (vertices_curr[...,-1].min(dim=-1).values - height_tolorance).min()
...
class MotionLib():
    def __init__(self, motion_file, key_body_ids, device, fix_height = True, masterfoot_conifg = None, min_length = -1, debug=False):
        self.debug = debug
