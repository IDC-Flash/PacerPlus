...
from amp.env.tasks.humanoid_amp_task import HumanoidAMPTask
...
class AMPAgent(common_agent.CommonAgent):
    def __init__(self, base_name, config):
        ...
        if pretrained_model_cp is not None and not config.get('load_checkpoint', False):
            print(f'Load pretrained model from {pretrained_model_cp}')
    ...
    def set_stats_weights(self, weights):
        ...
        if self._normalize_amp_input:
            if 'amp_input_mean_std' in weights:
    ...
    def _sym_loss(self, flip_obs_batch, orig_obs_batch):
        ...
        flip_a = flip_a.reshape(B, -1, 3)[..., :len(left_to_right_index), :].reshape(B, -1)
    ...
    def _disc_loss(self, disc_agent_logit, disc_demo_logit, obs_demo):
        ...
        '''
        # prediction loss
        disc_loss_agent = self._disc_loss_neg(disc_agent_logit).mean(-1, keepdim=True)
        disc_loss_demo = self._disc_loss_pos(disc_demo_logit).mean(-1, keepdim=True)
    ...
    def _calc_disc_rewards(self, amp_obs):
        ...
        return disc_r.mean(-1, keepdim=True)
    ...
    def load_pretrained(self, cp_path):
        ...
        load_rlgame_norm = len([key for key in missing_checkpoint_keys if 'running_obs' in key]) > 0 and 'running_mean_std' in checkpoint
        if load_rlgame_norm:
