...
class HumanoidPedestrianTerrain(humanoid_traj.HumanoidTraj):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id,
        ...
        self.use_trajectory_velocity = cfg["env"].get("useTrajectoryVelocity", False)
    ...
    def get_task_obs_size(self):
        ...
        if (self._enable_task_obs):
            ...
            if self.use_trajectory_velocity:
                obs_size += 2 * (self._num_traj_samples - 1)
            if self.terrain_obs:
    ...
    def get_task_obs_size_detail(self):
        task_obs_detail = []
        if (self._enable_task_obs):
            num_trajectories = self._num_traj_samples * 2 - 1 if self.use_trajectory_velocity else self._num_traj_samples
            task_obs_detail.append(["traj", 2 * num_trajectories])
    ...
    def contact_force_plotters(self, w, plotter_names, identifier='', pause_time=0.00000001):
        ...
        if not hasattr(self, 'lines'):
            ...
            # update plot label/title
            plt.ylabel('Scaled Contact Force')
    ...
    def _compute_task_obs(self, env_ids=None):
        ...
        if self.use_trajectory_velocity:
            trajectory_velocity = traj_samples[:, 1:] - traj_samples[:, :-1]
            traj_samples = torch.cat([traj_samples, trajectory_velocity], dim=1)

        obs = compute_location_observations(root_states, traj_samples, self._has_upright_start, self.use_trajectory_velocity)
        if self.terrain_obs:
    ...
    def _compute_flip_task_obs(self, normal_task_obs, env_ids):
        ...
        traj_samples_dim = 20 if not self.use_trajectory_velocity else 20 + 18
        obs_acc = []
        normal_task_obs = normal_task_obs.clone()
        traj_samples = normal_task_obs[:, :traj_samples_dim].view(B, traj_samples_dim//2, 2).clone()
        traj_samples[..., 1] *= -1
    ...
    def create_training_ground(self):
        ...
        # asset_file = "sgst_bigstair_rt338.urdf"
        # archway_options = gymapi.AssetOptions()
        # archway_options.disable_gravity = True
        # archway_options.fix_base_link = True
        # archway_options.vhacd_enabled = True
        # asset_root = 'data/stairs'
        # archway_options.vhacd_params.resolution = 20000
        # archway_options.vhacd_params.max_convex_hulls = 5
        # archway_options.vhacd_params.max_num_vertices_per_ch = 64
        # test_obj = self.gym.load_asset(self.sim, asset_root, asset_file, archway_options)
        # default_pose_scene = gymapi.Transform()
        # default_pose_scene.p = gymapi.Vec3(0,0,1)
        # self._scene_asset = test_obj
...
from amp.utils.draw_utils import *
...
# Task-location
@torch.jit.script
def compute_location_observations(root_states, traj_samples, upright, has_traj_velocity=False):
    # type: (Tensor, Tensor, bool, bool) -> Tensor
    ...
    if has_traj_velocity:
        traj_length = traj_samples.shape[1] // 2 + 1
        traj_samples_delta = traj_samples[:, :traj_length, :] - root_pos.unsqueeze(-2)
        traj_velocity_data = traj_samples[:, traj_length:, :]
        traj_samples_delta = torch.cat([traj_samples_delta, traj_velocity_data], dim=1)
    else:
