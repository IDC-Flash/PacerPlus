...
from uhc.utils.rotation_conversions import axis_angle_to_matrix
...
class Humanoid_Batch:
    ...
    def update_model(self, betas, gender):
        ...
        self._offsets = torch.from_numpy(
            np.round(np.stack(off_sets, axis=1), decimals=5))
    ...
    def fk_batch(self, pose, trans, convert_to_mat=True, count_offset=True):
        ...
        if convert_to_mat:
            pose_mat = axis_angle_to_matrix(pose.reshape(
                B * seq_len, -1, 3)).reshape(B, seq_len, -1, 3, 3)
        else:
            pose_mat = pose
        if pose_mat.shape != 5:
...
if __name__ == "__main__":
    ...
    return_dict = humanoid_batch.fk_batch(pose_aa_smpl[None, ],
                                          trans[None, ],
                                          count_offset=count_offset)
    diff = return_dict['wbpos'].reshape(-1, 24, 3) - fk_res['wbpos'].reshape(
        -1, 24, 3)
    print(diff.abs().sum())
    import ipdb
    ipdb.set_trace()
