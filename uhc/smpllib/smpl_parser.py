...
class SMPLH_Parser(_SMPLH):
    ...
    def get_offsets(self, zero_pose=None, betas=torch.zeros(1, 16).float()):
        with torch.no_grad():
            if zero_pose is None:
                verts, jts = self.get_joints_verts(self.zero_pose, th_betas=betas)
            else:
                verts, jts = self.get_joints_verts(zero_pose, th_betas=betas)
            verts_np = verts.detach().cpu().numpy()
            ...
            skin_weights = self.lbs_weights.numpy()
            return (verts[0], jts_np[0], skin_weights, self.joint_names,
                    offset_smpl_dict, parents_dict, channels, self.joint_range)

    def get_mesh_offsets(self, zero_pose=None, betas=torch.zeros(1, 16), scale=None, flatfoot=False):
        with torch.no_grad():
            joint_names = self.joint_names
            if zero_pose is None:
