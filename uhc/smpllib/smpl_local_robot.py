...
def update_joint_limits(joint_range):
    ...
    joint_range["R_Hip"][2] = np.array([-np.pi / 3, np.pi / 3])
...
def update_joint_limits_upright(joint_range):
    ...
    joint_range["L_Knee"][1] = np.array([-np.pi, np.pi])
    joint_range["L_Knee"][2] = np.array([-np.pi / 32, np.pi / 32])

    joint_range["R_Knee"][0] = np.array([-np.pi / 32, np.pi / 32])
    joint_range["R_Knee"][1] = np.array([-np.pi, np.pi])
    joint_range["R_Knee"][2] = np.array([-np.pi / 32, np.pi / 32])
...
class Robot:
    def __init__(self, cfg, data_dir="data/smpl", model_xml_path=None, create_default_skeleton=False, clean_up=False):
        self.bodies = []
        self.weight = 0
        self.height = 0
        self.cfg = cfg
        ...
        self.rel_joint_lm = cfg.get("rel_joint_lm",
                                    True)  # Rolling this out worldwide!!
...
if __name__ == "__main__":
    ...
    # smpl_robot.write_xml(f"amp/data/assets/mjcf/mesh_humanoid.xml")
    # model = load_model_from_path(f"amp/data/assets/mjcf/mesh_humanoid.xml")
    ...
    # model = load_model_from_path(f"amp/data/assets/mjcf/amp_humanoid.xml")
