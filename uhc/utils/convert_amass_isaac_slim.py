...
# amass_data = joblib.load("/hdd/zen/data/ActBound/AMASS/amass_copycat_take5_5.pkl")
# amass_data = joblib.load("/hdd/zen/data/ActBound/AMASS/amass_copycat_take5_train.pkl")
amass_data = joblib.load("/hdd/zen/data/ActBound/AMASS/standing_male.pkl")
...
for key_name in tqdm(amass_data.keys()):
    ...
    # if not key_name in amss_run_data and not key_name in amass_crawl_data :
    #     continue
    # if not key_name in amass_crawl_data :
    #     continue
    # if not key_name in amss_run_data:
        # continue

    # if not key_name in amss_run_data or "stairs" in key_name:
    #     continue
    # if not key_name in amss_walk_data:
    #     continue
    # if not "crawl" in key_name:
    #     continue
    # if not "0-ACCAD_Female1Running_c3d_C5 - walk to run_poses" in key_name:
    #     continue
    # if not(np.sum(smpl_data_entry['trans'][..., 2] < 0.3) > 30):
    #     continue
    ...
    for idx in range(num):
        pose_quat = sRot.from_rotvec(pose_aa_mj.reshape(-1,
                                                        3)).as_quat().reshape(
                                                            batch_size, 24, 4)
        ...
        skeleton_tree = SkeletonTree.from_mjcf("amp/data/assets/mjcf/smpl_humanoid_1.xml")
...
import ipdb
ipdb.set_trace()
# joblib.dump(amass_full_motion_dict, "data/amass/pkls/amass_isaac_im_train_upright_slim.pkl")
# joblib.dump(amass_full_motion_dict, "data/amass/pkls/amass_isaac_run_upright_slim.pkl")
# joblib.dump(amass_full_motion_dict, "data/amass/pkls/amass_isaac_simple_run_upright_slim.pkl")
# joblib.dump(amass_full_motion_dict, "data/amass/pkls/amass_isaac_walk_upright_test_slim.pkl")
# joblib.dump(amass_full_motion_dict, "data/amass/pkls/amass_isaac_slowalk_upright_slim.pkl")
# joblib.dump(amass_full_motion_dict, "data/amass/pkls/amass_isaac_crawl_upright_slim.pkl")
# joblib.dump(amass_full_motion_dict, "data/amass/pkls/amass_isaac_run_crawl_upright_slim.pkl")
joblib.dump(amass_full_motion_dict, "data/amass/pkls/amass_isaac_standing_upright_slim.pkl")